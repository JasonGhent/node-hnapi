  ...
  if (!/[<>]/.test(body)){
    ...
    try {
      ...
      for (var i=0, l=rows.length; i<l; i+=2){
        ...
        var timeAgo = userLink[0] ? userLink[0].nextSibling.textContent.replace('|', '').trim() : '';
        var commentsCount = parseInt(cell2.find('a[href^=item]').text(), 10) || 0;
    ...
    try {
      ...
      read(url,function(err,article,meta){
        if(err) fn(err);
          ...
          article.title='<h3>'+article.title+'</h3><br />';
          post.content=article.title+article.content;
          fn(null, post);
        }
      });
    } catch (e){
